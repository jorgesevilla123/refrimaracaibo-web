...
@Component({
  selector: 'app-hogar-categories',
  templateUrl: './hogar-categories.component.html',
  styleUrls: ['./hogar-categories.component.scss']
})
export class HogarCategoriesComponent implements OnInit {
  category: any
    ...
    this.productsService.filterCategory(category, page, 'HOGAR').subscribe({
      next: (res) => {
        let parentRoute = this.route.parent.snapshot.routeConfig.path
          this.paginationService.parentCategory = 'HOGAR'
